...
Image {
    ...
    function displayText(value) {
        if (isVehicleMarker(value)) {
            if (UIListener.getCurrentScreen() == 1)
                return "Vehicle"
            return qsTranslate( "main", stockMarkerText(value) )
        }
        return qsTranslate( "main", value )
    }

    function displayImage(value, image) {
        if (isVehicleMarker(value) && UIListener.getCurrentScreen() == 1) {
            if (image.indexOf("/ico_home_180_") != -1)
                return "/app/share/images/AppHome/main/ico_home_180_vehicle"
            if (image.indexOf("/ico_home_196_") != -1)
                return "/app/share/images/AppHome/main/ico_home_196_vehicle"
            if (image.indexOf("/ico_home_234_") != -1)
                return "/app/share/images/AppHome/main/ico_home_234_vehicle"
        }
        return image
    }
    ...
    function itemClicked()
    {
        ...
        else if( bEnabled )
        {
            ...
            */
            ...
            if(EngineListener.getCallState() == 1 && 
                ( nAppId == EHSDefP.APP_ID_SETTINGS_SOUND
                  || nAppId == EHSDefP.APP_ID_SETTINGS_BLUETOOTH
                  || nAppId == EHSDefP.APP_ID_SETTINGS_GENERAL_SYSTEM))
            {
                ViewControll.ShowPopUpConnectivity( EHSDefP.POPUP_DISABLE_DURING_CALL);
                return;
            }
            
            if(EngineListener.getCallState() == 2 && 
                ( nAppId == EHSDefP.APP_ID_SETTINGS_SOUND
                  || nAppId == EHSDefP.APP_ID_SETTINGS_BLUETOOTH
                  || nAppId == EHSDefP.APP_ID_SETTINGS_GENERAL_SYSTEM
                  || nAppId == EHSDefP.APP_ID_SETTINGS_CONNECTIVITY))
            {
                ViewControll.ShowPopUpConnectivity( EHSDefP.POPUP_DISABLE_HOLD_CALL);
                return;
            }
    ...
    source: displayImage(sText, sImage) + iconItem.sSourceSuffix
    ...
    Text {
        ...
        text: displayText(sText) + LocTrigger.empty
    ...
    Connections {
        ...
        onRetrigger: {
            iconItemText.text = displayText(sText) + LocTrigger.empty
